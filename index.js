...
var mhAuth = Cypress.env('mailpitAuth');
...
var fetchMessage = function (id) {
    return cy.request({
        method: 'GET',
        url: mhApiUrl("/v1/message/".concat(id)),
        auth: mhAuth,
    })
        .then(function (response) {
        if (typeof response.body === 'string') {
...
Cypress.Commands.add('mhGetBody', { prevSubject: true }, function (mail) {
...
Cypress.Commands.add('mhGetRecipients', { prevSubject: true }, function (mail) {
    var recipients = [];
    mail.To.map(function (recipientObj) {
